 ...
 *
 * Unchanged apart from the `"use client"` directive above, which the App Router
 * needs and the upstream Vite source has no concept of, and the file-scoped
 * eslint-disable below.
 *
 * Options are laid out along a circle whose radius keeps the arc between two
 * neighbours equal to one row height, so `tilt` alone controls how tightly the
 * list curls. Wheel, drag, click and arrow keys all drive the same target
 * value, which a single rAF loop eases towards with frame-rate independent
 * smoothing.
 *
 * The rules below are disabled for this file only. Upstream writes its config
